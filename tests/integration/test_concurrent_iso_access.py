...
def atomic_file_write(temp_file_path, target_path):
    ...
    try:
        ...
        print(f"  ✓ Atomic file write completed: {os.path.basename(target_path)}")
...
def simulate_iso_download(vm_id, url, storage_path, filename):
    ...
    with file_lock:
        ...
        if os.path.exists(iso_path):
            ...
            with tempfile.NamedTemporaryFile() as tmp:
                ...
                if existing_hash_hex:
                    print(f"VM {vm_id}: Hash comparison - Downloaded: {downloaded_hash_hex[:8]}, Existing: {existing_hash_hex[:8]}")
                    
                    if downloaded_hash_hex == existing_hash_hex:
                        print(f"VM {vm_id}: ✓ Files identical, reusing existing file")
                        return f"local:iso/{filename}"
                    else:
                        print(f"VM {vm_id}: Files differ, creating unique filename")
                    ...
                    print(f"VM {vm_id}: ⚠ Could not calculate hash, proceeding with upload")
        else:
        ...
        print(f"VM {vm_id}: ✓ ISO ready: local:iso/{filename}")
        return f"local:iso/{filename}"
...
def simulate_vm_iso_mount(vm_id, url, storage_path):
    ...
    try:
        ...
        print(f"VM {vm_id}: Operation completed in {end_time - start_time:.2f}s")
        print(f"VM {vm_id}: Result: {result}")
...
def main():
    ...
    print(f"Starting {vm_count} concurrent VM operations...")
    print("Each VM will try to download/mount the same ISO simultaneously.\n")
    
    for vm_id in range(1, vm_count + 1):
        thread = threading.Thread(
            target=simulate_vm_iso_mount,
            args=(vm_id, test_url, storage_path)
        )
    ...
    print(f"\n=== Test Results ===")
    print(f"All {vm_count} VM operations completed!")
