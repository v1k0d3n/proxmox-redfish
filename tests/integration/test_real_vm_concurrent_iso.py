...
skip_if_no_real_vms = pytest.mark.skipif(
    os.environ.get("REAL_VM_TESTS") != "1", reason="Skipping real VM tests unless REAL_VM_TESTS=1"
)
...
def test_virtual_media_insert(vm_id, iso_url):
    ...
    try:
        ...
        payload = {"Image": iso_url, "Inserted": True}
...
def simulate_concurrent_iso_operations(vm_id, iso_url, operation_delay=2):
    ...
    """
    log("Starting concurrent ISO operation simulation", vm_id)
